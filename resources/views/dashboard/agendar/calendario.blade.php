  ...
  > div {
    ...
    &:hover .content {
      ...
      visibility: visible;
        ...
        <dialog class="container-xxl" id="dialog">
        ...
        <a class="o-btn" href="javascript:;">AGENDAR</a>
      </div>
    </div>
  </header>
<div class="c-calendar">
    <div class="c-calendar__style c-aside">
      ...
      <div style="display: block" class="c-aside__day">
...
function preencherHorarios(horariosDisponiveis) {
    ...
    if (Array.isArray(horariosDisponiveis)) {
        ...
        horariosDisponiveis.forEach(function(horario) {
            ...
            divHorario.classList.add('btn', 'btn-outline-primary', 'm-2','selecaoHorario'); // Adiciona classes Bootstrap para estilização
            // Adiciona o bloco de horário ao contêiner
            horariosContainer.appendChild(divHorario);

        });
    } else {
        console.error('horariosDisponiveis não é um array:', horariosDisponiveis);
    }
}
    ...
    function capturarHorarioSelecionado() {
    // Seleciona todos os elementos de horário disponíveis
    const horariosDisponiveis = document.querySelectorAll('.selecaoHorario');

    // Itera sobre cada elemento de horário disponível
    horariosDisponiveis.forEach(function(horario) {
        // Adiciona um evento de clique a cada elemento de horário disponível
        horario.addEventListener('click', function() {
            // Recupera o horário do texto do elemento clicado
            const horarioSelecionado = this.textContent;

            // Agora você pode fazer o que quiser com o horário selecionado, como armazená-lo em uma variável ou enviá-lo para o servidor via AJAX
